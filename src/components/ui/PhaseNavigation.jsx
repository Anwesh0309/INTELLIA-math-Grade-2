...
export const PhaseNavigation = () => {
  ...
  const phaseSequence = [
    ...
    { key: PHASES.PRACTICE, label: 'Practice', icon: '🎮', num: '04' },
  ...
  // Do not show navigation or mute button in the intro phase
  if (currentPhase === PHASES.INTRO) return null;

  return (
    <div className="flex items-center justify-center gap-3 z-50 pointer-events-auto" id="phase-navigation-container">
              ...
              >
                <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-black transition-all ${
                  isActive
                    ...
                    : 'bg-transparent border border-slate-500 text-slate-300 hover:border-slate-300'
                }`}>
                  {isCompleted ? (
                ...
                <span className={`text-[11px] font-extrabold tracking-wider flex items-center gap-1 ${
                  isActive ? 'text-[#FFD100]' : isCompleted ? 'text-[#34D399]' : 'text-slate-300 hover:text-white'
                }`}>
                  <span className="opacity-90">{phase.icon}</span> {phase.label}
                </span>
              </button>
            </div>
          );
      ...
      {/* Mute button beside the nav bar */}
      <AudioToggle />
    </div>
  );
};
